...
import { tanggalLokalHariIni, tanggalAwalBulanLokal } from '@/lib/tanggal';
...
export default function RekapAbsensiPage() {
  ...
  useEffect(() => {
    fetch('/api/kelas', { cache: 'no-store' }).then((r) => r.json()).then((d) => { setKelasList(d); if (d[0]) setKelasId(String(d[0].id)); });
  }, []);
  ...
  const muatRekap = () => {
    ...
    fetch(`/api/absensi/rekap?${params}`, { cache: 'no-store' })
      .then(async (r) => {
        const d = await r.json();
        if (idPermintaanIni !== requestIdRef.current) return; // abaikan response yang sudah usang
        if (!r.ok) {
          setFetchError(d.error || 'Gagal memuat data rekap dari server');
          setData([]);
          return;
        }
        setData(d.data || []);
        setNamaKelas(d.kelas || '');
      })
      .catch(() => {
        if (idPermintaanIni !== requestIdRef.current) return;
        setFetchError('Tidak bisa terhubung ke server. Periksa koneksi lalu coba lagi.');
        setData([]);
      })
      .finally(() => {
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Button variant="secondary" onClick={muatRekap} disabled={loading}>
              {loading ? 'Memuat...' : 'Muat Ulang'}
            </Button>
            <Button onClick={downloadPdf} disabled={data.length === 0}>Download PDF</Button>
          </div>
        }
      />
      ...
      <Card className="overflow-x-auto">
        {loading ? (
          ...
          <div className="py-14 text-center px-4">
            <p className="text-sm text-red-700 bg-red-50 border border-red-100 rounded-lg px-4 py-2.5 inline-block">{fetchError}</p>
            <div className="mt-3">
              <Button variant="secondary" onClick={muatRekap}>Coba Lagi</Button>
            </div>
          </div>
        ) : data.length === 0 ? (
