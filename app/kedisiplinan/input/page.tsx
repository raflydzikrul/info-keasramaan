...
export default function InputPelanggaranPage() {
  ...
  useEffect(() => {
    fetch('/api/kelas', { cache: 'no-store' }).then((r) => r.json()).then(setKelasList);
    fetch('/api/kategori-pelanggaran', { cache: 'no-store' }).then((r) => r.json()).then(setKategoriList);
  }, []);

  useEffect(() => {
    if (!kelasId) { setSiswaList([]); return; }
    fetch(`/api/siswa?kelas_id=${kelasId}`, { cache: 'no-store' }).then((r) => r.json()).then(setSiswaList);
  }, [kelasId]);
