...
export default function KelasPage() {
  ...
  const load = () => fetch('/api/kelas', { cache: 'no-store' }).then((r) => r.json()).then(setList);
  useEffect(() => { load(); }, []);

  const resetForm = () => { setNama(''); setWali(''); setEditId(null); setFormError(''); };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(url, { method, cache: 'no-store', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nama, wali_kelas: wali }) });
      const data = await res.json();
      if (!res.ok) {
        setFormError(data.error || 'Gagal menyimpan data kelas');
        return;
      }
      resetForm();
      load();
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`/api/kelas/${id}`, { method: 'DELETE', cache: 'no-store' });
    load();
  };
