...
export default function JenisAbsensiPage() {
  ...
  const load = () => {
    apiGet<Jenis[]>('/api/jenis-absensi')
      .then(setList)
      .catch((e) => setError(pesanError(e)));
  };
  useEffect(() => { load(); }, []);

  const resetForm = () => { setNama(''); setKategori('Sunnah'); setEditId(null); setError(''); };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await apiSend(url, editId ? 'PUT' : 'POST', { nama: nama.trim(), kategori });
      resetForm();
      load();
    } catch (err) {
  ...
  const edit = (j: Jenis) => { setEditId(j.id); setNama(j.nama); setKategori(j.kategori); setError(''); };

  const remove = async (id: number) => {
    if (!confirm('Hapus jenis absensi ini? Seluruh riwayat absensi untuk jenis ini juga akan terhapus.')) return;
    setError('');
    try {
      await apiSend(`/api/jenis-absensi/${id}`, 'DELETE');
