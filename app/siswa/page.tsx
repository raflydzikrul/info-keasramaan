...
export default function SiswaPage() {
  ...
  const load = () => {
    ...
    if (q) params.set('q', q);
    fetch(`/api/siswa?${params}`, { cache: 'no-store' }).then((r) => r.json()).then((d) => {
      if (idPermintaanIni !== requestIdRef.current) return; // abaikan response yang sudah usang
      setList(d);
    });
  };

  useEffect(() => { fetch('/api/kelas', { cache: 'no-store' }).then((r) => r.json()).then(setKelasList); }, []);
  useEffect(() => { load(); }, [filterKelas, q]);

  const resetForm = () => { setNis(''); setNama(''); setKelasId(''); setJk('L'); setEditId(null); setFormError(''); };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(url, { method, cache: 'no-store', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok) {
        setFormError(data.error || 'Gagal menyimpan data santri');
        return;
      }
      resetForm();
      load();
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`/api/siswa/${id}`, { method: 'DELETE', cache: 'no-store' });
    load();
  };
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch('/api/siswa/import', { method: 'POST', cache: 'no-store', body: formData });
      const data = await res.json();
      if (!res.ok) {
