...
import { Card, StatCard, Badge, PageHeader, EmptyState, Button } from '@/components/ui';
import { apiGet, pesanError } from '@/lib/api';
import { useRefetchOnFocus } from '@/lib/use-refetch-on-focus';

type DashboardData = {
  totalSiswa: number;
  totalKelas: number;
  absensiHariIni: { hadir: number; terlambat: number; tidak_hadir: number; total: number };
...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="mb-5 flex flex-wrap items-center gap-3 text-sm text-red-700 bg-red-50 border border-red-100 rounded-lg px-4 py-3">
          <span>{error}</span>
          <Button variant="secondary" onClick={muat}>Coba Lagi</Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          sub={data
            ? `${data.absensiHariIni.hadir} hadir${data.absensiHariIni.terlambat ? ` · ${data.absensiHariIni.terlambat} terlambat` : ''} dari ${data.absensiHariIni.total} catatan`
            : undefined}
        />
        <StatCard
